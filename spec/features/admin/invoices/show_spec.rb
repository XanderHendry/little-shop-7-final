...
RSpec.describe "Admin Invoice Show" do
  ...
  context "Bulk-Discounts" do
    ...
    describe "total discounted revenue on Admin Invoice Show page" do 
      it "displays the total discounted revenue on an invoice, next to the total revenue" do 
        visit "/admin/invoices/#{@c2_invoice3.id}"

      expect(page).to have_content("Invoice ##{@c2_invoice3.id}")
      expect(page).to have_content("Customer Name: #{@c2_invoice3.customer.first_name} #{@c2_invoice3.customer.last_name}")
      expect(page).to have_content("Status: #{@c2_invoice3.status}")
      expect(page).to have_content("Created on: #{@c2_invoice3.created_at.strftime("%A, %B %d, %Y")}")
      expect(page).to have_content("Total Revenue: $525.00")
      expect(page).to have_content("Total Discounted Revenue: $412.50")
      expect(page).to have_content("#{@m2_item2.name}")
      expect(page).to have_content("Quantity: #{@invoice_item9.quantity}")
      expect(page).to have_content("Price: $#{@invoice_item9.unit_price_show}")
      expect(page).to have_content("Status: #{@invoice_item9.status}")
      expect(page).to have_content("#{@m4_item3.name}")
      expect(page).to have_content("Quantity: #{@invoice_item10.quantity}")
      expect(page).to have_content("Price: $#{@invoice_item10.unit_price_show}")
